...
public class BowlOfCandy {
	...
	//TODO -completed make the method below - takeCandy() 
	public synchronized void takeCandy(String candyName) throws InterruptedException {
		while (bowlWithCandy.size() == 0) {
			System.out.println("the bowl is empty: " + bowlWithCandy.size() + " " + Thread.currentThread().getName()
					+ " is going to wait.");
			System.out.println();
			//TODO - completedmake the thread hold until it is notified by another thread that it can continue to work
			wait();
		}
		removeCandyFromTheBowl(candyName);
		// TODO -completedlet the other thread know that it is ok to do the work on this object now
		notifyAll();
		System.out.println(Thread.currentThread().getName() + " has notified the other thread.");
		System.out.println();
	}

	//TODO -completed make the method below - refillCandy synchronized
	public synchronized void  refillCandy(List<String> packOfCandy) throws InterruptedException {
		while (bowlWithCandy.size() == 3 && packOfCandy.size() > 0) {
			...
			System.out.println();
			//TODO -completed make the thread hold until it is notified by another thread
			wait();
		}
		...
		// TODO -completed let the other thread know that it is ok to do the work on this object now
		notifyAll();
		System.out.println(Thread.currentThread().getName() + " has notified the other thread.");
		System.out.println();
	}
